...
class CellsInterface:
    ...
    def compute_T_f_gradT_f_quick(self):
        ...
        Tim32, dTdxim32, _ = self._interp_lagrange_amont(self.Tg[0], self.Tg[1], self.Tg[2],
                                                         ...
                                                         -0.5 * self.dx)
        Tim12, _, _ = self._interp_lagrange_amont(self.Tg[1], self.Tg[2], self.Ti,
        ...
        _, dTdxip12, _ = self._interp_lagrange_centre_grad(self.Td[2], self.Td[1], self.Ti, self._dTdxd,
                                                           ...
                                                           0.5 * self.dx)
        Tip32, dTdxip32, _ = self._interp_lagrange_amont(self.Ti, self.Td[1], self.Td[2],
    ...
    def compute_T_f_gradT_f_upwind(self):
        ...
        """
        Tim32, dTdxim32, _ = self._interp_upwind(self.Tg[1], self.Tg[2],
                                                 -1. * self.dx, 0. * self.dx)
        Tim12, dTdxim12, _ = self._interp_upwind(self.Tg[-1], self.Tg[0],
                                                 -1. * self.dx, 0. * self.dx)
        Tip12, dTdxip12, _ = self._interp_upwind(self.Td[0], self.Td[1],
                                                 0. * self.dx, 1. * self.dx)
        Tip32, dTdxip32, _ = self._interp_upwind(self.Td[1], self.Td[2],
                                                 1. * self.dx, 2. * self.dx)
        Tip52, dTdxip52, _ = self._interp_upwind(self.Td[2], self.Td[3],
                                                 2. * self.dx, 3. * self.dx)
        self._T_f[0] = np.nan  # on ne veut pas se servir de cette valeur, on veut utiliser la version weno / quick
        self._T_f[1] = Tim32
        self._T_f[2] = Tim12  # self._T_dlg(0.)
    ...
    def compute_from_h_T(self, h: float, T_mean: float):
        ...
        if self.ag < EPSILON:
            ...
            self._Ti, self._lda_gradTi = self._get_T_i_and_lda_grad_T_i(T_mean, self.Td[1], self.ag / 2. * self.dx,
                                                                        (1.+self.ad) / 2. * self.dx)
        else:
    ...
    @staticmethod
    def _interp_lagrange_centre_grad(Tgg: float, Tg: float, Ti: float, gradTi: float, xgg: float, xg: float, xi: float, x_face: float) \
            -> (float, float, float):
        """
